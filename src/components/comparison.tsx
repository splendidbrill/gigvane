import { IconArrowRight, IconCheck, IconDatabase, IconPen, IconVideo, IconX } from "./icons";
...
const COMPARES = [
  {
    ...
    gigvane: {
      price: 50,
      time: "2 hr",
      notes: ["Auto-spec'd from your raw clips", "Instant iterations · word-edit", "Pay 10% to start, 90% on accept"],
    },
    speedX: 48,
    costPct: 83,
  },
  {
    ...
    gigvane: {
      price: 75,
      time: "10 min",
      notes: ["Real-time pulled & verified", "SMTP + LinkedIn check", "Receipts attached per row"],
    },
    speedX: 1008,
    costPct: 91,
  },
  {
    ...
    gigvane: {
      price: 150,
      time: "30 min",
      notes: ["Brand voice trained on your blog", "Schema + internal-links wired", "Originality > 96% verified"],
    },
    speedX: 672,
    costPct: 90,
  },
...
function CompareRow({ row, index }: { row: (typeof COMPARES)[number]; index: number }) {
  const Icon = row.Icon;
  return (
    <div
      className="compare-row grid grid-cols-12 gap-0"
    ...
    >
      ...
      <div className="col-span-12 lg:col-span-3 px-6 py-7 flex flex-col gap-2">
        ...
        <div className="mt-3 flex flex-col gap-2">
          <div
            className="flex items-baseline justify-between font-mono text-[10.5px]"
            style={{ color: "#6e6c63", letterSpacing: ".04em" }}
          >
            <span>SPEED-UP</span>
          ...
          <div
            className="flex items-baseline justify-between font-mono text-[10.5px] mt-1"
            style={{ color: "#6e6c63", letterSpacing: ".04em" }}
          >
            <span>COST-CUT</span>
      ...
      {/* Old way */}
      <div
        className="col-span-12 lg:col-span-4 px-6 py-7"
        ...
        <div className="mt-3 flex items-baseline gap-3">
          <div>
            ...
            <div className="text-[34px] font-semibold tk-tighter tnum" style={{ color: "#0E0E10" }}>
              <span style={{ textDecoration: "line-through", textDecorationThickness: "1.5px", textDecorationColor: "#A0A099" }}>
                ${row.human.price}
              </span>
            </div>
          </div>
          ...
          {row.human.notes.map((n, i) => (
            <li key={i} className="flex items-start gap-2 text-[13.5px]" style={{ color: "#56544c" }}>
              <span
                ...
                style={{ width: 5, height: 5, borderRadius: 9999, background: "#A0A099" }}
              />
              {n}
            </li>
          ))}
      ...
      {/* Gigvane way */}
      <div
        className="col-span-12 lg:col-span-5 px-6 py-7 relative"
      ...
      >
        ...
        <div className="mt-3 flex items-baseline gap-6">
          <div>
            <div className="eyebrow">Price</div>
            <div className="text-[44px] font-semibold tk-tightest tnum leading-none">${row.gigvane.price}</div>
          </div>
          <div>
        ...
        <ul className="mt-4 flex flex-col gap-1.5">
          {row.gigvane.notes.map((n, i) => (
            <li key={i} className="flex items-start gap-2 text-[13.5px]" style={{ color: "#2a2a2c" }}>
              <span className="mt-[5px] shrink-0" style={{ color: "#0E0E10" }}>
        ...
        <div className="grid lg:grid-cols-[1fr,1.2fr] gap-10 mb-12">
          <div>
            ...
            <h2 className="mt-3 text-[44px] sm:text-[56px] font-semibold tk-tightest leading-[1.0]">
              The slowest, most expensive part of your team —{" "}
              <span className="volt-under">replaced</span>.
            </h2>
          </div>
