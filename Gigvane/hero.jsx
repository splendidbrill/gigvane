// hero.jsx — Navbar + Hero + Magic Input → SOW demo
const { useState, useEffect, useRef, useMemo } = React;

// ── Wordmark ────────────────────────────────────────────────────────────
function Wordmark({ size = 26, dark = false }) {
      ...
      <div className="max-w-[1240px] mx-auto h-[64px] px-6 lg:px-10 flex items-center justify-between">
        <div className="flex items-center gap-10">
          ...
          <div className="hidden md:flex items-center gap-7 text-[13.5px]" style={{ color:'#2a2a2c' }}>
            ...
            <a href="#manifesto" className="hover:opacity-70">Manifesto</a>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <a href="#" className="hidden sm:inline text-[13.5px] hover:opacity-70" style={{ color:'#2a2a2c' }}>Sign in</a>
          <a href="#waitlist" className="btn-ink inline-flex items-center gap-1.5">
...
const PROMPTS = [
  {
    ...
    sow: {
      ...
      eta: { val: '2 hr 12 min', etaSec: 7920 },
      price: 50, human: { price: 300, eta: '4 days' },
  ...
  },
  {
    ...
    sow: {
      ...
      eta: { val: '10 min', etaSec: 600 },
      price: 75, human: { price: 800, eta: '1 week' },
  ...
  },
  {
    ...
    sow: {
      ...
      eta: { val: '34 min', etaSec: 2040 },
      price: 150, human: { price: 1500, eta: '2 weeks' },
...
function MagicInput() {
  ...
  const [phase, setPhase] = useState('typing'); // typing → thinking → sow
  const [held, setHeld] = useState(false); // pause auto-rotate on hover
  ...
  const switchTo = (i) => { setIdx(i); setPhase('typing'); };

  return (
    <div className="w-full max-w-[860px] mx-auto"
         onMouseEnter={() => setHeld(true)} onMouseLeave={() => setHeld(false)}>
      {/* Outer surface — paper card */}
      <div className="card-bone p-3 sm:p-4" style={{ borderRadius: 22 }}>
        ...
        {/* prompt input */}
        <div className="rounded-2xl px-4 sm:px-5 py-4 sm:py-5"
             style={{ background:'#FFFFFF', border:'1px solid var(--hair)' }}>
          <div className="flex items-start gap-3">
            <span className="mt-1 shrink-0" style={{ color:'#0E0E10', opacity:.5 }}>
              <IconCommand size={18}/>
            </span>
            <div className="text-[17px] sm:text-[19px] leading-[1.45] tk-tight flex-1"
            ...
            <button className="btn-ink shrink-0 inline-flex items-center gap-1.5 self-start">
          ...
          <div className="mt-4 pt-3 flex flex-wrap items-center gap-2"
            ...
            {PROMPTS.map((p, i) => (
              <button key={p.key} onClick={() => switchTo(i)}
        ...
        <div className="mt-3 relative" style={{ minHeight: 220 }}>
          {phase === 'thinking' && (
            <div className="rounded-2xl p-5 flex items-center gap-3 rise"
                 style={{ background:'#FBF9F2', border:'1px solid var(--hair)' }}>
              <div className="flex gap-1">
                {[0,1,2].map(i => (
                  <span key={i} style={{
                    width:6, height:6, borderRadius:9999, background:'#0E0E10', opacity:.7,
                    animation:`pulse 1.2s ease-in-out ${i*0.15}s infinite`,
                  }}/>
                ))}
              </div>
              <span className="text-[14px]" style={{ color:'#3a3a3c' }}>
                Routing to <span className="font-mono text-[12.5px]">{cur.sow.agents[0]}</span> · scoring 6 candidate agents…
              </span>
            </div>
          )}
      ...
      {/* dotted instructions below the dock */}
      <div className="mt-3 flex flex-wrap items-center justify-center gap-x-5 gap-y-1 text-[12.5px]"
           style={{ color:'#5d5b53' }}>
...
function SOWCard({ sow }) {
  return (
    <div className="rise rounded-2xl overflow-hidden"
         style={{ background:'#0E0E10', color:'#F4F1E8', border:'1px solid #1c1c20',
                  boxShadow:'0 1px 0 rgba(255,255,255,.05) inset, 0 18px 50px -18px rgba(14,14,16,.55)' }}>
      {/* SOW header */}
      <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom:'1px solid #1c1c20' }}>
        <div className="flex items-center gap-3">
          <span className="font-mono text-[11px]" style={{ color:'#9c9a90', letterSpacing:'.04em' }}>{sow.id}</span>
          <span className="font-mono text-[11px]" style={{ color:'#62615a' }}>·</span>
          <span className="text-[14px] font-medium tk-tight">{sow.title}</span>
        </div>
        <span className="font-mono text-[10.5px] inline-flex items-center gap-1.5"
      ...
      <div className="grid md:grid-cols-[1.4fr,1fr]">
        ...
        <div className="px-5 py-4" style={{ borderRight:'1px solid #1c1c20' }}>
          <div className="font-mono text-[10.5px] mb-2" style={{ color:'#62615a', letterSpacing:'.08em' }}>DELIVERABLES</div>
          <ul className="flex flex-col">
            {sow.lines.map((l, i) => (
              <li key={i} className="flex items-start gap-2.5 py-2"
                  style={{ borderTop: i ? '1px solid #1c1c20' : 'none' }}>
                <span className="mt-1 shrink-0" style={{ color:'#CFF24E' }}><IconCheck size={13} stroke={2.4}/></span>
          ...
          {/* agents assigned */}
          <div className="mt-3 pt-3 flex items-center gap-2 flex-wrap" style={{ borderTop:'1px dashed #2a2a30' }}>
            <span className="font-mono text-[10.5px]" style={{ color:'#62615a', letterSpacing:'.08em' }}>AGENT FLEET</span>
            {sow.agents.map((a, i) => (
              <span key={i} className="font-mono text-[10.5px] px-2 py-[3px] rounded-full"
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3 text-[12.5px]" style={{ color:'#bfbeb4' }}>
            <span className="inline-flex items-center gap-1.5"><IconClock size={13}/> ETA <b className="tnum text-[#F4F1E8]">{sow.eta.val}</b></span>
          ...
          {/* 10/90 split bar */}
          <div className="mt-1">
            <div className="flex justify-between font-mono text-[10.5px]" style={{ color:'#80807a', letterSpacing:'.04em' }}>
              <span>10% to start</span><span>90% on delivery</span>
            </div>
            <div className="mt-1.5 h-[10px] rounded-full overflow-hidden flex" style={{ background:'#1c1c20' }}>
            ...
            <div className="mt-1.5 flex justify-between font-mono text-[10.5px] tnum" style={{ color:'#bfbeb4' }}>
              <span>${(sow.price*0.1).toFixed(2)}</span>
              <span>${(sow.price*0.9).toFixed(2)}</span>
            </div>
          </div>

          <button className="btn-volt mt-1 w-full inline-flex items-center justify-center gap-1.5">
            Authorize 10% · Start work <IconArrowRight size={14}/>
          </button>
          <div className="font-mono text-[10.5px] text-center" style={{ color:'#62615a', letterSpacing:'.04em' }}>
...
function Hero() {
  return (
    <section className="relative overflow-hidden" data-screen-label="01 Hero">
      <div className="grain"/>
      <div className="max-w-[1240px] mx-auto px-6 lg:px-10 pt-16 lg:pt-24 pb-16">
        ...
        {/* Headline */}
        <h1 className="text-center mx-auto max-w-[1080px] tk-tightest font-semibold"
            style={{ fontSize:'clamp(48px, 8vw, 104px)', lineHeight: 0.96 }}>
          Buy the outcome.<br/>
          <span className="italic font-normal" style={{ fontFamily:'"Instrument Serif", serif', letterSpacing:'-0.02em' }}>
            Skip&nbsp;the&nbsp;freelancer.
          </span>
        </h1>

        {/* Subhead */}
        <p className="mt-7 mx-auto max-w-[680px] text-center text-[18px] sm:text-[19px] leading-[1.5] tk-tight"
           style={{ color:'#3a3a3c' }}>
        ...
        <div className="mt-8 flex items-center justify-center gap-3">
          ...
          <a href="#how" className="btn-ghost inline-flex items-center gap-1.5">
            See the 10/90 model
          </a>
        </div>
        ...
        <div className="mt-12 mx-auto max-w-[920px] grid grid-cols-2 sm:grid-cols-4 gap-px overflow-hidden rounded-2xl"
...
Object.assign(window, { Navbar, Hero, Wordmark, MagicInput });
